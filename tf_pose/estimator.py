...
class TfPoseEstimator:
    ...
    @staticmethod
    #######################改进################################3
    def draw_humans(npimg, humans, prehumans, imgcopy=False):
        if imgcopy:
            npimg = np.copy(npimg)
            # 获取图片的高和宽
        ...
        if prehumans != []:
            for human, prehuman in zip(humans, prehumans):
                # draw point
                for i in range(common.CocoPart.Background.value):
                    if i not in human.body_parts.keys():
                        if i not in prehuman.body_parts.keys():
                            continue
                        if i in prehuman.body_parts.keys():
                            body_part = prehuman.body_parts[i]
                    # 取出对应的关键点
                ...
                for pair_order, pair in enumerate(common.CocoPairsRender):
                    ...
                    if pair[0] not in human.body_parts.keys() or pair[1] not in human.body_parts.keys():
                        if pair[0] not in prehuman.body_parts.keys() or pair[1] not in prehuman.body_parts.keys():
                            continue
                        if pair[0] in prehuman.body_parts.keys() or pair[1] not in prehuman.body_parts.keys():
                            cv2.line(npimg, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)
                    # 用直线将关节画出来
                    if pair[0] in human.body_parts.keys() or pair[1] not in human.body_parts.keys():
                        cv2.line(npimg, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)
